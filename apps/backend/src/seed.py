...
async def seed() -> None:
    ...
    async with dbmod.SessionLocal() as session:  # type: AsyncSession
        ...
        # --- Bulk Movies Seed (rich dataset for /movies) ---
        # Ensure we have at least ~80 movies with diverse fields so the frontend can demo filters
        from sqlalchemy import func as _func, select as _select
        total_movies = (await session.execute(_select(_func.count(Movie.id)))).scalar_one()
        if total_movies < 60:
            ...
            countries = [
                "USA","UK","France","India","Japan","South Korea","Italy","Germany","Spain","Brazil",
                "Canada","Australia","China","Sweden","Denmark","Norway","Mexico","Turkey","Iran","Russia",
            ]
            languages = [
                "en","fr","hi","ja","ko","it","de","es","pt","zh","sv","da","no","tr","fa","ru"
            ]
            posters = [
                "/placeholder.svg?height=400&width=300&text=Poster",
                "/action-movie-poster.png",
                "/animated-movie-poster.png",
                "/dramatic-city-rooftop.png",
                "/sci-fi-neon.png",
            ]
            # Ensure a few more genres are present
            for slug, name in [
                ("thriller","Thriller"), ("romance","Romance"), ("action","Action"), ("horror","Horror"), ("adventure","Adventure"),
                ("biopic","Biopic"), ("animation","Animation"), ("documentary","Documentary"), ("mystery","Mystery"),
            ]:
            ...
            # Convert genre values to list
            all_genres = list(by_slug.values())
            ...
            for idx in range(1, 90):
                ...
                year = str(_rand.choice(years))
                runtime = _rand.randint(85, 170)
                siddu_score = round(_rand.uniform(4.0, 9.9), 1)
                language = _rand.choice(languages)
                country = _rand.choice(countries)
                poster_url = _rand.choice(posters)
                m = Movie(
                    external_id=ext,
                    title=title,
                    year=year,
                    ...
                    language=language,
                    country=country,
                    overview=f"An engaging story of #{idx} spanning genres and eras.",
                    poster_url=poster_url,
                )
                session.add(m)
                await session.flush()
                # attach 1-3 genres randomly via association table to avoid async lazy-load
                from sqlalchemy import insert as _insert
                from .models import movie_genres as _movie_genres
                gcount = _rand.randint(1, 3)
                chosen = _rand.sample(all_genres, gcount)
                for g in chosen:
                    await session.execute(_insert(_movie_genres).values(movie_id=m.id, genre_id=g.id))
                created += 1
            if created:
                await session.commit()
