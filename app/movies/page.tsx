...
export default function MoviesPage() {
  ...
  useEffect(() => {
    ...
    if (filters.yearRange[0]) params.set("yearMin", String(filters.yearRange[0]))
    if (filters.yearRange[1]) params.set("yearMax", String(filters.yearRange[1]))
    if (filters.countries.length) params.set("countries", filters.countries.join(","))
    if (filters.languages.length) params.set("languages", filters.languages.join(","))
    if (filters.scoreRange[0] !== 0) params.set("ratingMin", String(filters.scoreRange[0]))
    if (filters.scoreRange[1] !== 10) params.set("ratingMax", String(filters.scoreRange[1]))
    if (sortBy) params.set("sortBy", sortBy)
    fetch(`${apiBase}/api/v1/movies?${params.toString()}`, { cache: "no-store" })
      .then(async (r) => {
