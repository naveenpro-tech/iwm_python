...
export const MovieCard: React.FC<MovieCardProps> = ({
  ...
  // Validate poster URL - ensure it's a valid HTTP/HTTPS URL or starts with /
  // Invalid URLs like /mnt/data/... should fallback to placeholder
  const getValidPosterUrl = () => {
    if (!posterUrl) return "/placeholder.svg?width=200&height=300&query=Movie+Poster"
    ...
    // Check if it starts with / but is not a local file path like /mnt/
    if (posterUrl.startsWith('/') && !posterUrl.startsWith('/mnt') && !posterUrl.startsWith('/tmp')) {
      return posterUrl
    }

    // Invalid URL, use placeholder
    return "/placeholder.svg?width=200&height=300&query=Movie+Poster"
  }
