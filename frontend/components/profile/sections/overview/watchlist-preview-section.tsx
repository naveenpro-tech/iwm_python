    ...
    const fetchWatchlist = async () => {
      ...
      try {
        ...
        const mapped: WatchlistMovie[] = items.slice(0, 9).map((w: any) => {
          // Extract poster URL safely
          let posterUrl = w.posterUrl || w.movie?.posterUrl || ""

          // Validate URL - it should start with http:// or https:// or /
          // If it contains JSON-like structure, try to parse it
          if (posterUrl && typeof posterUrl === 'string' && posterUrl.includes('"')) {
            // Attempt to extract URL from malformed JSON string
            const urlMatch = posterUrl.match(/["']([^"']+\.(jpg|jpeg|png|webp|gif|svg))["']/i)
            posterUrl = urlMatch ? urlMatch[1] : ""
          }

          // Filter out invalid local paths
          if (posterUrl && (posterUrl.startsWith('/mnt') || posterUrl.startsWith('/tmp'))) {
            posterUrl = ""
          }
