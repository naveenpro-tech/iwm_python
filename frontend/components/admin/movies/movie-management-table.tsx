...
export function MovieManagementTable({
  ...
  const sanitizePosterUrl = (url: string | null | undefined): string => {
    if (!url) return "/placeholder.svg?height=72&width=48&query=movie+poster"
    ...
    if (typeof url === 'string' && url.includes('"')) {
      ...
      if (urlMatch) {
        ...
        // Validate extracted URL - must be http/https or valid web path
        ...
        return "/placeholder.svg?height=72&width=48&query=movie+poster"
      }
            ...
            >
              ...
              <TableCell className="px-3 py-2">
                <div className="relative w-12 h-[72px] rounded overflow-hidden bg-muted">
                  <Image
                    src={sanitizePosterUrl(movie.poster)}
                    alt={movie.title}
                    fill
